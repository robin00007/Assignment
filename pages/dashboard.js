...
const Dashboards = () => {
  ...
  console.log(session);
  // if(!session && typeof window !== 'undefined'){router.push('/')}
  return (
    <div className="h-screen w-screen color-primary-white p-5 flex flex-row">
      <div className="bg-primary h-full w-1/5 rounded-lg mr-3 p-10 flex flex-col text-white">
        ...
        <div className="flex-1">
          <div className="text-lg mb-2 flex items-center"><RxDashboard style={{marginRight:"15px"}}/>Dashboard</div>
          <div className="text-lg mb-2 flex items-center"><BsFillTagsFill style={{marginRight:"15px"}}/>Transaction</div>
          <div className="text-lg mb-2 flex items-center"><BsCalendar2DateFill style={{marginRight:"15px"}} fill="white" stroke="white" color="white"/>Scheudle</div>
          <div className="text-lg mb-2 flex items-center"><GoPeople style={{marginRight:"15px"}}/>Users</div>
          <div className="text-lg mb-2 flex items-center"><AiFillSetting style={{marginRight:"15px"}}/>Settings</div>
        </div>
        <div>
          <p>Help</p>
          <p>Contact Us</p>
      ...
      <div className="h-full flex-1 rounded-lg p-6 flex flex-col">
        <div className="flex">
          ...
          <div className="flex items-center">
            <input
              ...
              placeholder="Search"
            />
            <div className="mx-2"><FaRegBell fill="black"/></div>
            {session?.user?.image && (
        ...
        <div className="flex h-1/5">
          <div className="bg-white text-black flex-1 my-3 mr-6 flex-col p-3 rounded-lg">
            <div className=""><BsCashCoin size={"2em"} color="#7FCD93" /></div>
          ...
          <div className="bg-white text-black flex-1 my-3 mr-6 flex-col p-3 rounded-lg">
            <div className="flex-1 w-full flex pr-5"><BsHandThumbsUp size={"2em"} color="
#ECA4A4"/></div>
            <div>total Likes</div>
            <div className="font-bold">3,83,484</div>
          </div>
          <div className="bg-white text-black flex-1 my-3 flex-col p-3 rounded-lg">
            <div className="flex-1 w-full flex pr-5"><GoPeople size={"2em"} color="#A9B0E5"/></div>
            <div>total Users</div>
            <div className="font-bold">3,83,484</div>
          </div>
        </div>
        ...
        >
          {data.map((entry, index) => (
            <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} styles={{borderRadius:'20px !important'}} />
          ))}
        </Pie>
      </PieChart>
            </ResponsiveContainer>
          ...
          <div className="bg-white flex-1 flex ml-2 rounded-lg text-black flex-col p-5">
            ...
            <p className="font-semibold my-4 text-sm text-gray-500">Add Profile</p>
            </div>
          </div>
        </div>
      </div>
